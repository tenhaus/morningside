        ...
        B = function(a) {
            ...
            function g(a, e, f, g, h) {
                ...
                i.url.split(".").pop().split("?").shift(), i.bypass || (e &&
                    (e = d(e) ? e : e[a] || e[g] || e[a.split("/").pop().split(
                        "?")[0]]), i.instead ? i.instead(a, e, f, g, h) : (
                        y[i.url] ? i.noexec = !0 : y[i.url] = 1, f.load(i.url,
                            i.forceCSS || !i.forceJS && "css" == i.url.split(
                                ".").pop().split("?").shift() ? "c" : c, i.noexec,
...
(function(compId) {
    ...
    function supportsRGBA() {
        ...
        return 0 == ("" + testEle.style.backgroundColor).indexOf("rgba") ? !
            0 : !1
    }
    var hasTransform = isSupported(["transformProperty", "WebkitTransform",
    ...
    function doLoadResources() {
        for (var a = 0; a < AdobeEdge._preloaders.length; a++) AdobeEdge._preloaders[
            a]()
    }
    ...
    function playWhenReady() {
        ...
        for (var a = 0; a < AdobeEdge._readyplayers.length; a++) AdobeEdge._readyplayers[
            a]()
    }
    ...
    function edgeCallback(a, d, b) {
        ...
        a == AdobeEdge.preload.last && (!AdobeEdge.bootstrapLoading ||
            AdobeEdge._playWhenReady ? AdobeEdge.okToLaunchComposition(
                compId) : readyToPlay = !0, AdobeEdge.preload.busy = !1, 0 <
            AdobeEdge.preload.q.length && (a = AdobeEdge.preload.q.pop(),
                AdobeEdge.requestResources(a.files, a.callback)))
    }
    AdobeEdge.requestResources = AdobeEdge.requestResources || function(a,
        d) {
        ...
        for (b = 0; b < k; b++) {
            c = a[b];
            if ("string" === typeof c) 0 === c.indexOf("//") && 0 ===
                window.location.href.indexOf("file://") && (c = "http:" + c),
            url = c, c = {
                load: url
            };
            else if (c.load && 0 === c.load.indexOf("//") && 0 === window.location
                .href.indexOf("file://") && (c.load = "http:" + c.load),
                url = c.yep || c.load, c.callback) {
                var g = c.callback;
            ...
            AdobeEdge.preload.got[url] || (f.push(c), AdobeEdge.preload.last =
                url)
        }
        f.length && AdobeEdge.yepnope(f)
    };
    var filesToLoad, dlContent, preContent, doDelayLoad, signaledLoading,
        loadingEvt, requiresSVG, htLookup = {}, aLoader, aEffectors,
        plSTF, ctrPlS, minPlW, maxPlW, plHeight, plWidth;

    function loadResources(a, d) {
        AdobeEdge.preload = AdobeEdge.preload || [];
        AdobeEdge.preload.q = AdobeEdge.preload.q || [];
        d || !isCapable() ? filesToLoad = a : AdobeEdge.preload.busy ?
            AdobeEdge.preload.q.push({
                files: a,
    ...
    function bindPSS(a, d) {
        if (!isWrapped(a)) {
            var b = function() {
                ...
                c /= h;
                g /= n;
                "both" === d ? e = Math.min(c, g) : "height" === d ? e = g :
                    "width" === d && (e = c);
                void 0 !== maxPlW && (e = Math.min(e, parseInt(maxPlW, 10) /
                    h));
                void 0 !== minPlW && (e = Math.max(e, parseInt(minPlW, 10) /
                    h));
                g = "scale(" + e + ")";
                ...
                l.mozTransform = g;
                l.oTransform = g;
                if (!f || b) parent(a).style.height = Math.round(n * e) +
                    "px", parent(a).style.width = Math.round(h * e) + "px";
                b && (b = parent(parent(a)), b.style.height = Math.round(n *
                    e + offset(a).top - offset(b).top))
            };
    ...
    function centerThePreloadStage(a, d) {
        ...
        var b = a.style;
        /^both$|^horizontal$/.test(d) && (b.position = "absolute", b.marginLeft =
            "auto", b.marginRight = "auto", b.left = "0", b.right = "0");
        /^both$|^vertical$/.test(d) && (b.position = "absolute", b.marginTop =
            "auto", b.marginBottom = "auto", b.top = "0", b.bottom = "0")
    }
    ...
    function simpleContent(a, d, b) {
        var k = document.getElementsByTagName("body")[0],
            ...
            c, g, h;
        f ? "absolute" != f.style.position && "relative" != f.style.position &&
            (f.style.position = "relative") : f = k;
        plHeight && (f.style.height = plHeight);
        plWidth && (f.style.width = plWidth);
        /^height$|^width$|^both$/.test(plSTF) && d && !b && bindPSS(f,
            plSTF);
        /^vertical$|^horizontal$|^both$/.test(ctrPlS) && d && !b &&
            centerThePreloadStage(f, ctrPlS);
        for (var n = 0; n < a.length; n++) {
            b = a[n];
            "image" === b.type ? (k = document.createElement("img"), k.src =
                b.fill[1]) : k = document.createElement("div");
            k.id = b.id;
            h = k.style;
            if ("text" == b.type) {
                if (c = b.font) c[0] && "" !== c[0] && (h.fontFamily = c[0]),
                    "object" != typeof c[1] && (c[1] = [c[1]]), c[1][1] ||
                    (c[1][1] = "px"), c[1][0] && "" !== c[1][0] && (h.fontSize =
                        c[1][0] + c[1][1]), c[2] && "" !== c[2] && (h.color =
                        safeColor(c[2])), c[3] && "" !== c[3] && (h.fontWeight =
                        c[3]), c[4] && "" !== c[4] && (h.textDecoration = b
                        .font[4]), c[5] && "" !== c[5] && (h.fontStyle = b.font[
                        5]);
                b.align && "auto" != b.align && (h.textAlign = b.align);
                b.position && (h.position = b.position);
                (!b.rect[2] || 0 >= b.rect[2]) && (!b.rect[3] || 0 >= b.rect[
                    3]) && (h.whiteSpace = "nowrap");
                k.innerHTML = b.text
            }
            d && (k.className = d);
            h.position = "absolute";
            ...
            if (b.transform && b.transform[0]) {
                ...
                if (1 < b.transform[0].length) {
                    m = b.transform[0][1];
                    (e = splitUnits(m)) && e.units && (m = e.num, "%" == e.units &&
                        b.rect[3] && (e = b.rect[3], (l = splitUnits(b.rect[
                                3])) && l.units && (e = l.num, "%" == l.units &&
                                (e = e / 100 * f.offsetHeight)), m = m /
                            100 * e, 0 < f.offsetHeight && (m = m / f.offsetHeight *
                                100)));
            ...
            b.linkURL && (htLookup[k.id] = b, k.onclick = function() {
                ...
                a.linkTarget ? window.open(a.linkURL, a.linkTarget) :
    ...
    window.AdobeEdge.bootstrapCallback = function(a) {
        ...
        if (0 < aBootcompsLoaded.length)
            for (var d = 0; d < aBootcompsLoaded.length; d++) a(
                aBootcompsLoaded[d])
    };
    ...
    window.AdobeEdge.preloadComplete[compId] = function(a) {
        ...
        for (var d = window.AdobeEdge.bootstrapListeners.length, b = 0; b <
            d; b++) try {
    ...
    function onDocLoaded(a) {
        ...
        isCapable() ? (preContent && preContent.dom && preContent.dom.length &&
            simpleContent(preContent.dom, "edgePreload" + compId),
            filesToLoad && !signaledLoading && (loadResources(filesToLoad),
                filesToLoad = void 0)) : dlContent && dlContent.dom && (
            loadingEvt && loadingEvt({
                event: "done",
                progress: 1,
                reason: "downlevel"
    ...
    aLoader = [{
        ...
        load: "/js/slider/MORNINGSIDE_edge.js"
    }, {
        load: "/js/slider/MORNINGSIDE_edgeActions.js"
    }];
